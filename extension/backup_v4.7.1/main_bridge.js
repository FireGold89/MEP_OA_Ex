        ...
        setInterval(() => {
            ...
            function searchAndClick(win) {
                try {
                    ...
                    // 已處理過的標籤，防止同一畫面的重複操作
                    if (fDoc.body && fDoc.body.getAttribute('oa-processed')) return true;

                    // 1. 定義目標查找，放寬比對條件（去除所有空白與換行，轉小寫）
                    const elements = Array.from(fDoc.querySelectorAll('td, span, a, div.e8_browser_item'));
                    const targetEl = elements.find(el => {
                        if (el.children.length > 2) return false; // 排除超大容器
                    ...
                    if (targetEl) {
                        ...
                        row.click();
                        // 觸發雙擊，很多彈窗雙擊即選中並關閉
                        const dblEvent = new MouseEvent('dblclick', { bubbles: true, cancelable: true, view: win });
